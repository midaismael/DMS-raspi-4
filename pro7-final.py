...
current_state = "Forward (head pose)"
start_time = time.time()
alert_displayed = False
...
while True:
    ...
    if key == ord('g'):
        if (GPIO.input(INPUT_PIN) == True): # Physically read the pin now
            ...
            cv2.putText(frames, text, (250, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3, cv2.LINE_AA)
        else:
            #print("Co Situation: Danger")
            text = "Gas Situation: Danger!"
            cv2.putText(frames, text, (250, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3, cv2.LINE_AA)
    cv2.imshow('Head Pose + Drowseness + Eye Tracking + Face points + Face recognition', frames)
    #-------------------------------------------
    #Gas Sen
    #Heart sen
